...
pub fn load_from_path(path: &Path) -> ImageResult<ImageBuffer<image::Rgba<u8>, Bytes>> {
...
pub fn load_from_bytes(bytes: &[u8]) -> image::ImageResult<ImageBuffer<image::Rgba<u8>, Bytes>> {
    let (width, height, pixels) = {
...
pub fn load_from_rgba(
    ...
    height: u32,
    pixels: &Bytes,
) -> image::ImageResult<ImageBuffer<image::Rgba<u8>, Bytes>> {
